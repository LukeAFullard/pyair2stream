...
def _plot_sensitivity(data: CommonData, df_sens: pd.DataFrame):
    """
    Generate a bar chart of the parameter sensitivities.

    Creates a plot (`sensitivity_analysis.png`) displaying the mean absolute
    change in water temperature caused by perturbing each active parameter.

    Parameters
    ----------
    data : CommonData
        The global configuration and path data.
    df_sens : pd.DataFrame
        Dataframe containing sensitivity metrics generated by the core
