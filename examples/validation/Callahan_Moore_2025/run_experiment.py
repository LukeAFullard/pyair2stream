...
# 1. Generate data
df = pd.read_excel('examples/validation/Callahan_Moore_2025/ts_all_58_simplified.xlsx', sheet_name='ts_all_58')
...
os.makedirs('examples/validation/Callahan_Moore_2025/data', exist_ok=True)
save_csv(df_calib, 'examples/validation/Callahan_Moore_2025/data/07EA004_calib.csv')

# 2. Generate configs
os.makedirs('examples/validation/Callahan_Moore_2025/configs', exist_ok=True)
os.makedirs('examples/validation/Callahan_Moore_2025/output', exist_ok=True)
...
for name, opts in configs.items():
    ...
    with open(f'examples/validation/Callahan_Moore_2025/configs/{name}.yaml', 'w') as f:
        f.write(f"""project_name: "Callahan_Moore_2025"
station_name: "{name}"
series: "c"
time_resolution: "1d"
version: 8
...
paths:
  input_data: "examples/validation/Callahan_Moore_2025/data/07EA004_calib.csv"
  output_dir: "examples/validation/Callahan_Moore_2025/output"
...
# 3. Run models
def run_model(name):
    path = f'examples/validation/Callahan_Moore_2025/configs/{name}.yaml'
...
def get_lit_params():
    df = pd.read_csv('examples/validation/Callahan_Moore_2025/a2s_8_parameter_values.csv')
...
def extract_results(run_name):
    ...
    out_file = f"examples/validation/Callahan_Moore_2025/output/1_{optimizer}_NSE_{run_name}_c_1d.out"
    metrics_file = f"examples/validation/Callahan_Moore_2025/output/goodness_of_fit_calibration_{optimizer}_NSE_{run_name}.csv"
...
report = "# Validation Analysis: Callahan & Moore (2025)\n\n"
...
with open('examples/validation/Callahan_Moore_2025/README.md', 'w') as f:
    f.write(report)

shutil.rmtree('examples/validation/Callahan_Moore_2025/output')
shutil.rmtree('examples/validation/Callahan_Moore_2025/data')
shutil.rmtree('examples/validation/Callahan_Moore_2025/configs')
