...
| V2 | Given the published parameters, does it reproduce the published model errors for every model version on three Swiss rivers? Does its own calibration fit at least as well (judged by the original program too), and find the published parameters? | Ties the package to the peer-reviewed results of Piccolroaz et al. (2016). |
