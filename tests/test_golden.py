...
class TestGoldenOutputs(unittest.TestCase):
    ...
    def test_rk2_version_8_golden(self):
        """
        Validates the RK2 numerical integration against the output of the Fortran code.
        """
        data = CommonData()
        ...
        np.testing.assert_allclose(data.Twat_mod[365:], golden_twat_mod, rtol=1e-2, atol=1e-2,
            err_msg="Twat_mod RK2 output did not match Fortran output exactly")

    def test_rk4_version_7_golden(self):
        """
        Validates the RK4 numerical integration for version 7 (parameter 3 disabled).
        """
        data = CommonData()
        ...
        data.Qmedia = np.float64(10.0)
        data.Tice_cover = np.float64(0.0)
        # Parameter 3 (index 3) is disabled in version 7
        data.par = np.array([1.0, 0.1, 0.1, 0.0, 1.0, 1.0, 0.5, 0.1], dtype=np.float64)
        ...
        np.testing.assert_allclose(data.Twat_mod[365:], golden_twat_mod, rtol=1e-2, atol=1e-2,
            err_msg="Twat_mod version 7 output did not match Fortran output exactly")
